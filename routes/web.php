...
// Subscription routes (accessible even when trial expires)
Route::prefix('dashboard/subscription')->middleware(['auth', 'verified'])->group(function () {
    Route::get('/', [\App\Http\Controllers\Apps\SubscriptionController::class, 'index'])->name('subscription.index');
    Route::post('/upgrade', [\App\Http\Controllers\Apps\SubscriptionController::class, 'upgrade'])->name('subscription.upgrade');
    Route::get('/checkout', [\App\Http\Controllers\Apps\SubscriptionController::class, 'checkout'])->name('subscription.checkout');
    Route::post('/pay', [\App\Http\Controllers\Apps\SubscriptionController::class, 'pay'])->name('subscription.pay');
    Route::get('/invoice', [\App\Http\Controllers\Apps\SubscriptionController::class, 'invoice'])->name('subscription.invoice');
    Route::get('/billing', [\App\Http\Controllers\Apps\SubscriptionController::class, 'billing'])->name('subscription.billing');
    Route::post('/admin-contact', [\App\Http\Controllers\Apps\SubscriptionController::class, 'saveAdminContact'])->name('subscription.admin-contact');
});

// Super Admin routes (global admin only)
Route::prefix('dashboard/super-admin')->middleware(['auth', 'verified'])->group(function () {
    Route::get('/', [\App\Http\Controllers\Apps\AdminDashboardController::class, 'index'])->name('super-admin.dashboard');
    Route::get('/stores', [\App\Http\Controllers\Apps\AdminDashboardController::class, 'stores'])->name('super-admin.stores');
    Route::post('/stores/{store}/extend-trial', [\App\Http\Controllers\Apps\AdminDashboardController::class, 'extendTrial'])->name('super-admin.stores.extend-trial');
    Route::post('/stores/{store}/change-plan', [\App\Http\Controllers\Apps\AdminDashboardController::class, 'changePlan'])->name('super-admin.stores.change-plan');
    Route::post('/stores/{store}/toggle-status', [\App\Http\Controllers\Apps\AdminDashboardController::class, 'toggleStatus'])->name('super-admin.stores.toggle-status');
    Route::post('/stores/{store}/suspend', [\App\Http\Controllers\Apps\AdminDashboardController::class, 'suspend'])->name('super-admin.stores.suspend');
    Route::post('/stores/{store}/resume', [\App\Http\Controllers\Apps\AdminDashboardController::class, 'resume'])->name('super-admin.stores.resume');
    Route::post('/stores/{store}/activate-manual', [\App\Http\Controllers\Apps\AdminDashboardController::class, 'activateManual'])->name('super-admin.stores.activate-manual');
    Route::get('/stores/{store}', [\App\Http\Controllers\Apps\AdminDashboardController::class, 'storeDetail'])->name('super-admin.stores.detail');

    // Plan management
    Route::get('/plans', [\App\Http\Controllers\Apps\PlanController::class, 'index'])->name('super-admin.plans');
    Route::put('/plans/{plan}', [\App\Http\Controllers\Apps\PlanController::class, 'update'])->name('super-admin.plans.update');
});
...
Route::group(['prefix' => 'dashboard', 'middleware' => ['auth', 'verified', 'store_active']], function () {
    ...
    Route::resource('/users', UserController::class)
        ...
        ->middlewareFor('store', ['permission:users-create', 'step_up', 'plan_limit:max_users'])
    ...
    Route::post('/users/{user}/impersonate', [UserController::class, 'impersonate'])->middleware('step_up')->name('users.impersonate');
    Route::post('/users/{user}/reset-password', [UserController::class, 'resetPassword'])->middleware('step_up')->name('users.reset-password');
    Route::post('/impersonate/stop', [UserController::class, 'stopImpersonate'])->name('users.impersonate.stop');
    ...
    Route::resource('products', ProductController::class)
        ...
        ->middlewareFor('store', ['permission:products-create', 'plan_limit:max_products'])
    ...
    // Stock Opname (require stock_opname plan feature)
    Route::middleware(['plan_feature:stock_opname'])->group(function () {
    ...
    // CRM features (require crm_automation plan feature)
    Route::middleware(['plan_feature:crm_automation'])->group(function () {
    ...
    // Sales Returns (require sales_returns plan feature)
    Route::middleware(['plan_feature:sales_returns'])->group(function () {
    ...
    // Purchase Orders (require purchase_orders plan feature)
    Route::middleware(['plan_feature:purchase_orders'])->group(function () {
    ...
    // route goods receivings (require purchase_orders plan feature)
    Route::middleware(['plan_feature:purchase_orders'])->group(function () {
    ...
    // route stock transfers (require multi_warehouse plan feature)
    Route::middleware(['plan_feature:stock_transfers'])->group(function () {
    ...
    // route supplier returns (require sales_returns plan feature)
    Route::middleware(['plan_feature:sales_returns'])->group(function () {
    ...
    Route::get('/settings/loyalty', [\App\Http\Controllers\Apps\SettingController::class, 'loyalty'])->middleware(['permission:dashboard-access', 'plan_feature:loyalty_program'])->name('settings.loyalty');
    Route::post('/settings/loyalty', [\App\Http\Controllers\Apps\SettingController::class, 'updateLoyalty'])->middleware(['permission:dashboard-access', 'plan_feature:loyalty_program'])->name('settings.loyalty.update');

    // settings whatsapp (require whatsapp_notifications plan feature)
    Route::middleware(['plan_feature:whatsapp_notifications'])->group(function () {
    ...
    Route::resource('/settings/price-lists', PriceListController::class)
        ...
        ->middlewareFor('store', ['permission:price-lists-create', 'step_up', 'plan_feature:price_lists'])
        ->middlewareFor('update', ['permission:price-lists-update', 'step_up', 'plan_feature:price_lists'])
        ->middlewareFor('destroy', ['permission:price-lists-delete', 'step_up', 'plan_feature:price_lists']);
    Route::post('/settings/price-lists/{priceList}/items', [PriceListController::class, 'updateItem'])->middleware(['permission:price-lists-update', 'step_up', 'plan_feature:price_lists'])->name('price-lists.items.update');
    Route::delete('/settings/price-lists/{priceList}/items/{productId}', [PriceListController::class, 'destroyItem'])->middleware(['permission:price-lists-update', 'step_up', 'plan_feature:price_lists'])->name('price-lists.items.destroy');
    ...
    Route::get('/reports/insights', [AdvancedSalesInsightsController::class, 'index'])->middleware(['permission:reports-access', 'plan_feature:advanced_reports'])->name('reports.insights.index');
...
Route::middleware('auth')->group(function () {
    Route::get('/store-picker', [\App\Http\Controllers\Apps\StorePickerController::class, 'index'])->name('store-picker.index');
    Route::post('/store-picker', [\App\Http\Controllers\Apps\StorePickerController::class, 'select'])->name('store-picker.select');
    Route::post('/switch-store', [\App\Http\Controllers\Apps\StorePickerController::class, 'switch'])->name('store-picker.switch');
});
