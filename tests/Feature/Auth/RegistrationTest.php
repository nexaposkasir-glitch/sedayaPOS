...
class RegistrationTest extends TestCase
{
    ...
    {
        Config::set('security.auth.public_registration', false);
    ...
    public function test_new_users_can_register_when_public_registration_is_enabled(): void
    {
        ...
        // Email is auto-verified in local testing, so redirect to dashboard
        $response->assertRedirect(route('dashboard'));
    }

    public function test_register_request_is_not_throttled_within_limit(): void
    {
        Config::set('security.auth.public_registration', true);

        // Throttle is now 100 per minute (AUTH_REGISTER_THROTTLE=100,1), so 4 attempts won't trigger
        for ($attempt = 1; $attempt <= 4; $attempt++) {
            ...
            // Each registration should succeed (redirect, not 429)
