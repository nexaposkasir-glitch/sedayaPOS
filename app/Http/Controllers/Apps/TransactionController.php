...
class TransactionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Partial reload: only cart data needed (for addToCart, updateCart, etc.)
        if ($request->header('X-Inertia-Partial-Data')) {
            $partialKeys = explode(',', $request->header('X-Inertia-Partial-Data'));

            if (in_array('carts', $partialKeys)) {
                return $this->cartPartialResponse($userId);
            }
        }
        ...
        // Cache active pricing rules once per request to avoid duplicate DB hits
        $activeRules = $this->pricingService->getActiveRules();

        $initialPricingPreview = $this->loyaltyService->previewCheckout(
            $this->pricingService->previewCartWithRules($carts, null, $activeRules)
        );
        ...
        $products = Product::with('category:id,name')
            ...
            ->orderBy('title')
            ->get();
        $pricingBadges = $this->pricingService->previewProductsWithRules($products, null, $activeRules);
    ...
    /**
     * Return only cart-related data for partial Inertia reloads.
     * Avoids re-querying products, categories, customers, etc.
     */
    protected function cartPartialResponse(int $userId): \Inertia\Response
    {
        $data = $this->getCartResponse();

        return \Inertia\Inertia::render('Dashboard/Transactions/Index', $data);
    }

    /**
     * Get cart data as array (carts, heldCarts, pricingPreview, totals).
     */
    protected function getCartResponse(): array
    {
        $userId = auth()->user()->id;
    ...
    public function addToCart(Request $request)
    {
        ...
        if ($product->is_composite) {
            ...
            foreach ($product->components as $component) {
                ...
                if ($isSuperAdmin && ! $warehouseId) {
                    // Superadmin without shift: use main product stock
                    $avail = $component->stock;
                } else {
                    // Normal cashier: use warehouse stock
        ...
        $cartData = $this->getCartResponse();

        return response()->json([
            'success' => true,
            'message' => 'Product Added Successfully!.',
            'carts' => $cartData['carts'],
            'carts_total' => $cartData['carts_total'],
            'heldCarts' => $cartData['heldCarts'],
            'initialPricingPreview' => $cartData['initialPricingPreview'],
        ]);
    }
            ...
            foreach ($carts as $cart) {
                ...
                if ($product->is_composite) {
                    ...
                    foreach ($product->components as $component) {
                        ...
                        if ($isSuperAdmin && ! $warehouseId) {
                            // Super admin without shift: only decrement main product stock
                            $component->decrement('stock', $componentQty);
                        } else {
                            // Normal cashier: decrement both warehouse and main product stock
