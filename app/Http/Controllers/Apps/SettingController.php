...
class SettingController extends Controller
{
    ...
    public function printer()
    {
        return Inertia::render('Dashboard/Settings/Printer', [
            'settings' => [
                ...
                'printer_cash_drawer' => Setting::getBool('printer_cash_drawer', false),
                'printer_device_id' => Setting::get('printer_device_id', ''),
                'printer_device_name' => Setting::get('printer_device_name', ''),
                'store_name' => Setting::get('store_name', 'SEDAYA POS'),
            ],
        ]);
    }
    ...
    public function updatePrinter(Request $request)
    {
        $validated = $request->validate([
            ...
            'printer_device_id' => ['nullable', 'string', 'max:255'],
            'printer_device_name' => ['nullable', 'string', 'max:255'],
        ]);
        ...
        Setting::set('printer_cash_drawer', ($validated['printer_cash_drawer'] ?? false) ? '1' : '0', 'Buka laci uang otomatis');

        if (array_key_exists('printer_device_id', $validated)) {
            Setting::set('printer_device_id', $validated['printer_device_id'] ?? '', 'ID device Bluetooth printer');
        }
        if (array_key_exists('printer_device_name', $validated)) {
            Setting::set('printer_device_name', $validated['printer_device_name'] ?? '', 'Nama device Bluetooth printer');
        }
