...
export default function Print({ transaction }) {
    ...
    useEffect(() => {
        const autoPrint = storeProfile?.printer_auto_print ?? false;
        if (!autoPrint) return;

        const deviceId = storeProfile?.printer_device_id ?? "";
        const paperSize = storeProfile?.printer_paper_size ?? "80mm";
        const cashDrawer = storeProfile?.printer_cash_drawer ?? false;

        async function doPrint() {
            try {
                const connected = isConnected();
                if (!connected && deviceId) {
                    await autoConnect(deviceId);
                }
                ...
                if (isConnected()) {
                    ...
                    // Disconnect after print to free Bluetooth
