...
export default function Index({
    ...
    const [heldCarts, setHeldCarts] = useState(initialHeldCarts);
    const [searchQuery, setSearchQuery] = useState("");
    const [selectedCategory, setSelectedCategory] = useState(null);

    // Sync with Inertia props when page reloads (hold, resume, checkout)
    useEffect(() => {
        setCarts(initialCarts);
        setCartsTotal(initialCartsTotal);
        setHeldCarts(initialHeldCarts);
    }, [initialCarts, initialCartsTotal, initialHeldCarts]);
    ...
    // Handle add product to cart — optimistic (instant UI)
    const handleAddToCart = async (product) => {
        if (!product?.id) return;

        setAddingProductId(product.id);

        // Optimistic: update cart locally first
        const existingIndex = carts.findIndex((c) => c.product_id === product.id || c.product?.id === product.id);
        let newCarts;
        if (existingIndex >= 0) {
            newCarts = carts.map((c, i) =>
                i === existingIndex
                    ? { ...c, qty: c.qty + 1, price: (c.price / c.qty) * (c.qty + 1) }
                    : c
            );
        } else {
            const newCart = {
                id: Date.now(), // temp ID, will be replaced by server
            ...
            newCarts = [newCart, ...carts];
        }
        const newTotal = newCarts.reduce((sum, c) => sum + Number(c.price), 0);
        setCarts(newCarts);
        setCartsTotal(newTotal);

        // Sync to server
        try {
            const { data } = await axios.post(route("transactions.addToCart"), {
                product_id: product.id,
            ...
            if (data.success) {
                setCarts(data.carts);
                setCartsTotal(data.carts_total || data.carts.reduce((s, c) => s + Number(c.price), 0));
    ...
    // Handle update cart quantity — optimistic
    const handleUpdateQty = (cartId, newQty) => {
        ...
        const newCarts = carts.map((c) =>
            c.id === cartId
                ? { ...c, qty: newQty, price: (c.price / c.qty) * newQty }
                : c
        );
        setCarts(newCarts);
        setCartsTotal(newCarts.reduce((s, c) => s + Number(c.price), 0));
    ...
    // Handle remove from cart — optimistic
    const handleRemoveFromCart = (cartId) => {
        setRemovingItemId(cartId);

        // If item is optimistic (temp ID, not yet synced to server), just remove locally
        const cartItem = carts.find((c) => c.id === cartId);
        if (cartItem?.optimistic) {
    ...
    if (!activeCashierShift && !isSuperAdmin) {
        return (
            <>
